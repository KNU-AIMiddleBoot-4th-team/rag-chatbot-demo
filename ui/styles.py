...
def inject_base_styles() -> None:
    ...
    st.markdown(
        ...
        /* stBottomBlockContainer를 화면 전체 폭으로 확장한다.
           Streamlit 기본값은 좁은 고정폭이라 입력창도 같이 좁아지는데,
           left:0/right:0/width:100% 로 덮어쓰면 화면 너비에 유동적으로 맞춰진다. */
        ...
        /* 입력창은 전체 폭 컨테이너 안에서 최대 800px, 좌우 여백 20px씩 중앙 정렬 */
        ...
        [data-testid="stChatInput"] textarea { background-color: transparent !important; color: #191F28 !important; caret-color: #3434e0 !important; font-size: 1rem !important; padding-left: 55px !important; max-height: 120px !important; overflow-y: auto !important; resize: none !important; white-space: pre-wrap !important; word-break: break-word !important; }
...
def _empty_state_layout() -> str:
    ...
    return """
        <style>
        /* 초기 화면: 하단 컨테이너(입력창 포함)를 화면 세로 중앙에 배치 */
        [data-testid="stBottomBlockContainer"] {
            bottom: 40% !important;
        }
        /* ➕ 첨부 버튼: 입력창 왼쪽 끝에 맞춤 */
        .st-key-attach_area {
            ...
            bottom: calc(40% + 8px) !important;
            left: max(20px, calc(50% - 380px)) !important;
        }
        /* FAQ: bottom은 JS(ResizeObserver)가 동적으로 계산해서 주입한다.
           여기서는 폭·정렬·z-index만 지정한다. */
        .st-key-faq_area {
            position: fixed !important;
            left: 50% !important;
...
def _chat_state_layout() -> str:
    """대화 중: 입력창은 Streamlit 기본 하단 고정, FAQ는 JS가 바로 위에 붙인다."""
    return """
        <style>
        /* 하단 영역(FAQ + 입력창)이 가리지 않도록 콘텐츠 하단 여백 확보.
           JS가 실제 높이를 계산해 padding-bottom을 덮어쓰므로 이 값은 fallback. */
